...
Добавете във сегашната йерархия клас SortFilter който трябва да връща редовете от входа сортирани лексикографски. Демонстрирайте как поне 3 от филтрите работят в един FilterChain обект последователно.

**За хората записали практикума**

* Реализирайте общ интерфейс за всички филтри.
* Направете така че FilterChain да поддържа всички филтри които имате до сега.
...
*Бонуси* - това са неща които са пожелание но носят допълнителни точки ако ги реализирате

* Направете изпълним файл за всеки от филтрите който работи така че да поддържа pipe-ване
    sort input-file.txt | filter-lines "The D programming language" | EncodeFilter | ZeroEscapeFilter > output-file.txt

* Напревете един общ изпълним файл който да поддържа следния интерфейс
    CLU input-file.txt output-file.txt --sort --filter-lines="The D programming language" --encode --zero-escape

Забележка: двата примера за дваа интерфейса би трябвало да имат еднакъв изход
