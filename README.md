...
Направете 5 класа които имплементират CustomSorter интерфейс и имплементират поне 5 различни алгоритъма, като 4 от тях са (heap, merge, quick, insertion).
```
class Sorter {
public:
...
Направете клас имплементиращ SortTester който пуска сортиране на всички Sorter обекти подадени в конструктора с данни които се генеррират от вашата имплементация. Метода getSummary извежда кратка статистика за пуснатите сортиращи алгоритми, времето за което те се изпълняват и данните върху които те са работили.
```
class SortTester {
public:
...
Към домашното си качете _кратко_ описание на данните които вашият тестов клас генерира. Опишете как се справят алгоритмите които сте имплементирали със генерираните от вас данни. Добавете и изхода от SortTester::getSummary.
